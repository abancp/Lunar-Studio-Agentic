...
export default function SidePanel({ agentStatus, activePage, onNavigate, isOpen, onClose }: SidePanelProps) {
    ...
    return (
        <>
            {/*
              On mobile: fixed drawer that slides in from the left, z-40.
              On md+: static inline sidebar.
            */}
            <aside
                className={`
                    flex flex-col h-full overflow-hidden glass-panel-solid rounded-xl
                    /* Mobile drawer */
                    fixed top-0 left-0 z-40 w-[280px] transition-transform duration-300 ease-in-out
                    md:static md:z-auto md:translate-x-0 md:w-56 lg:w-72 md:rounded-xl md:shrink-0
            ...
            >
                ...
                <div className="md:hidden flex items-center justify-between px-4 pt-4 pb-2">
                    <span className="text-xs font-semibold text-text-muted uppercase tracking-widest">Navigation</span>
                    <button
                        onClick={onClose}
                        className="p-2 rounded-lg hover:bg-bg-hover transition-colors text-text-secondary cursor-pointer"
                ...
                <div className="p-3 md:p-4 flex gap-2">
                    <div className="flex-1 flex items-center gap-2.5 px-3 py-2.5 rounded-lg bg-bg-tertiary/50 border border-border-default focus-within:border-accent-primary/40 transition-colors">
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-3 py-2.5 md:px-4 md:py-3 rounded-lg mb-1 transition-all duration-200 cursor-pointer group
                    ${isActive
                            ...
                            {toolNames.map((name) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <span className="text-xs text-text-secondary group-hover:text-text-primary flex-1 font-medium truncate">
                                            {name}
                                        </span>
                ...
                <div className="p-3 md:p-4 border-t border-border-default">
                    <div className="flex items-center gap-3 px-3 md:px-4 py-3 rounded-lg bg-bg-tertiary/40 border border-border-default">
                        <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center ${agentStatus?.whatsapp === 'connected' ? 'bg-success/10' : 'bg-text-muted/10'
                            }`}>
                            <Smartphone size={13} className={
